import type { Metadata } from 'next';
import { ProjectsView } from '@/components/pages/ProjectsView';
import { JsonLd } from '@/components/seo/JsonLd';
import { loadLocalePage } from '@/lib/i18n/loadPage';
import { t } from '@/lib/i18n/locale';
import { breadcrumbJsonLd } from '@/lib/seo/jsonld';
import { brandedTitle, buildPageMetadata } from '@/lib/seo/metadata';

export async function generateMetadata({
  params,
}: {
  params: Promise<{ locale: string }>;
}): Promise<Metadata> {
  const { locale, content } = await loadLocalePage(params);
  return buildPageMetadata({
    locale,
    path: '/projects',
    title: brandedTitle(t(content.home.projectsTitle, locale), content.settings.brandName),
    description: t(content.home.projectsSubtitle, locale),
    image: content.projects.find((p) => p.visible)?.images[0],
  });
}

export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  const { locale, content } = await loadLocalePage(params);
  return (
    <>
      <JsonLd
        data={breadcrumbJsonLd(locale, [
          { name: content.nav[locale].home, path: '/' },
          { name: content.nav[locale].projects, path: '/projects' },
        ])}
      />
      <ProjectsView locale={locale} content={content} />
    </>
  );
}
